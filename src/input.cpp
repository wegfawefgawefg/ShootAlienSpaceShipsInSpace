...
void process_event(SDL_Event& ev) {
    ...
    case SDL_KEYDOWN:
        ...
        if (ss->mode == ids::MODE_TITLE && ss->menu.capture_action_id >= 0) {
            ...
            save_input_bindings_to_ini(
                std::string("binds/") + ss->menu.binds_current_preset + ".ini", ss->input_binds);
...
void collect_menu_inputs() {
    ...
    auto ensure_gamepad = [&]() {
        if (gpad && SDL_GameControllerGetAttached(gpad))
            return;
        // Close stale
        if (gpad) {
    ...
    auto gp_btn = [&](SDL_GameControllerButton b) {
        return gpad && SDL_GameControllerGetButton(gpad, b) != 0;
    };
    auto gp_axis = [&](SDL_GameControllerAxis a) {
        return gpad ? SDL_GameControllerGetAxis(gpad, a) : 0;
    };
    ...
    bool gp_left =
        gp_btn(SDL_CONTROLLER_BUTTON_DPAD_LEFT) || (gp_axis(SDL_CONTROLLER_AXIS_LEFTX) < -DZ);
    bool gp_right =
        gp_btn(SDL_CONTROLLER_BUTTON_DPAD_RIGHT) || (gp_axis(SDL_CONTROLLER_AXIS_LEFTX) > DZ);
    bool gp_up =
        gp_btn(SDL_CONTROLLER_BUTTON_DPAD_UP) || (gp_axis(SDL_CONTROLLER_AXIS_LEFTY) < -DZ);
    bool gp_down =
        gp_btn(SDL_CONTROLLER_BUTTON_DPAD_DOWN) || (gp_axis(SDL_CONTROLLER_AXIS_LEFTY) > DZ);
    bool gp_confirm = gp_btn(SDL_CONTROLLER_BUTTON_A);
    bool gp_back = gp_btn(SDL_CONTROLLER_BUTTON_B);
    bool gp_page_prev = gp_btn(SDL_CONTROLLER_BUTTON_LEFTSHOULDER) ||
                        (gp_axis(SDL_CONTROLLER_AXIS_TRIGGERLEFT) > DZ);
    bool gp_page_next = gp_btn(SDL_CONTROLLER_BUTTON_RIGHTSHOULDER) ||
                        (gp_axis(SDL_CONTROLLER_AXIS_TRIGGERRIGHT) > DZ);

    // Build held (raw) states and edge events for confirm/back; directions used for repeat in menu
    // logic
    MenuInputs held{};
    ...
    if (held.left || held.right || held.up || held.down || out.confirm || out.back ||
        held.page_prev || held.page_next) {
        ss->menu.mouse_last_used = false;
        bool kb_any = kb_left || kb_right || kb_up || kb_down || kb_confirm || kb_back ||
                      kb_page_prev || kb_page_next;
