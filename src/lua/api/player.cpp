            ...
            if (g_mgr && g_state_ctx) {
                if (auto const* ad = g_mgr->find_ammo(ammo_type))
                    g_state_ctx->alerts.push_back(
                        {std::string("Ammo set: ") + ad->name, 0.0f, 1.2f, false});
                else
                    g_state_ctx->alerts.push_back(
                        {std::string("Ammo set: ") + std::to_string(ammo_type), 0.0f, 1.0f, false});
            }
        }
    });
    ...
    api.set_function("set_equipped_ammo_force", [](int ammo_type) {
        ...
        if (auto* gi = g_state_ctx->guns.get(*g_player_ctx->equipped_gun_vid)) {
            ...
            if (g_mgr && g_state_ctx) {
                if (auto const* ad = g_mgr->find_ammo(ammo_type))
                    g_state_ctx->alerts.push_back(
                        {std::string("Ammo forced: ") + ad->name, 0.0f, 1.2f, false});
                else
                    g_state_ctx->alerts.push_back(
                        {std::string("Ammo forced: ") + std::to_string(ammo_type), 0.0f, 1.0f,
                         false});
            }
        }
    });
