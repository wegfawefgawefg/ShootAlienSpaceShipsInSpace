...
void pre_physics_ticks() {
            ...
            if (pinv)
                for (const auto& entry : pinv->entries) {
                    ...
                    if (!gd || !luam->has_gun_on_step(gd->type))
                        continue;
                    if (gd->tick_rate_hz <= 0.0f || gd->tick_phase == std::string("after"))
                        continue;
                    gi->tick_acc += dt;
            ...
            if (pinv)
                for (const auto& entry : pinv->entries) {
                    ...
                    if (!idf || !luam->has_item_on_tick(idf->type))
                        continue;
                    if (idf->tick_rate_hz <= 0.0f || idf->tick_phase == std::string("after"))
                        continue;
                    inst->tick_acc += dt;
    ...
    if (luam) {
        ...
        for (auto& e : ss->entities.data()) {
            if (!e.active || e.def_type == 0)
                continue;
            const auto* ed = luam->find_entity_type(e.def_type);
            if (!ed)
                continue;
            if (ed->tick_rate_hz <= 0.0f || ed->tick_phase == std::string("after") ||
                !luam->has_entity_on_step(ed->type))
                continue;
...
void post_physics_ticks() {
    ...
    if (plat && luam) {
        ...
        int tick_calls = 0;
        // Guns with on_step
        if (auto* inv = ss->inv_for(*ss->player_vid))
            for (const auto& entry : inv->entries) {
                ...
                if (!gd || !luam->has_gun_on_step(gd->type))
                    continue;
                if (gd->tick_rate_hz <= 0.0f || gd->tick_phase == std::string("before"))
                    continue;
                gi->tick_acc += dt;
                float period = 1.0f / std::max(1.0f, gd->tick_rate_hz);
                while (gi->tick_acc >= period && tick_calls < MAX_TICKS) {
        ...
        if (auto* inv = ss->inv_for(*ss->player_vid))
            for (const auto& entry : inv->entries) {
                ...
                if (!idf || !luam->has_item_on_tick(idf->type))
                    continue;
                if (idf->tick_rate_hz <= 0.0f || idf->tick_phase == std::string("before"))
                    continue;
                inst->tick_acc += dt;
                float period = 1.0f / std::max(1.0f, idf->tick_rate_hz);
                while (inst->tick_acc >= period && tick_calls < MAX_TICKS) {
