...
static inline void ui_draw_kv_line(int tx, int& ty, int lh, const char* key,
    ...
    int keyw = 0, keyh = 0;
    SDL_Texture* kt = nullptr;
    if (ks) {
        kt = SDL_CreateTextureFromSurface(renderer, ks);
        SDL_QueryTexture(kt, nullptr, nullptr, &keyw, &keyh);
        SDL_Rect kd{tx, ty, keyw, keyh};
        SDL_RenderCopy(renderer, kt, nullptr, &kd);
    ...
    if (vs) {
        ...
        int vw = 0, vh = 0;
        SDL_QueryTexture(vt, nullptr, nullptr, &vw, &vh);
        SDL_Rect vd{tx + keyw, ty, vw, vh};
...
void render() {
        ...
        for (int y = 0; y < (int)ss->stage.get_height(); ++y) {
            for (int x = 0; x < (int)ss->stage.get_width(); ++x) {
                ...
                if (t.blocks_entities() || t.blocks_projectiles() || is_start || is_exit) {
                    ...
                    float scale = TILE_SIZE * gg->play_cam.zoom;
                    SDL_Rect tr{(int)std::floor(p0.x), (int)std::floor(p0.y), (int)std::ceil(scale),
                                (int)std::ceil(scale)};
    ...
    if (ss->mode == ids::MODE_PLAYING)
        for (auto const& e : ss->entities.data()) {
            ...
            if (!drew_sprite) {
                ...
                SDL_FPoint c = world_to_screen(e.pos.x - ds.x * 0.5f, e.pos.y - ds.y * 0.5f);
                float scale = TILE_SIZE * gg->play_cam.zoom;
                SDL_Rect r{(int)std::floor(c.x), (int)std::floor(c.y), (int)std::ceil(ds.x * scale),
                           (int)std::ceil(ds.y * scale)};
            ...
            if (e.type_ == ids::ET_PLAYER) {
                ...
                if (pl->equipped_gun_vid.has_value() && luam) {
                    ...
                    glm::vec2 m = {
                        gg->play_cam.pos.x +
                            (static_cast<float>(ss->mouse_inputs.pos.x) - (float)ww * 0.5f) *
                                inv_scale,
                        gg->play_cam.pos.y +
                            (static_cast<float>(ss->mouse_inputs.pos.y) - (float)wh * 0.5f) *
                                inv_scale};
    ...
    if (ss->mode == ids::MODE_PLAYING) {
        ...
        for (std::size_t i = 0; i < ss->ground_guns.data().size(); ++i) {
            ...
            if (!ggun.active)
                continue;
            SDL_FPoint c =
                world_to_screen(ggun.pos.x - ggun.size.x * 0.5f, ggun.pos.y - ggun.size.y * 0.5f);
            float scale = TILE_SIZE * gg->play_cam.zoom;
            SDL_Rect r{(int)std::floor(c.x), (int)std::floor(c.y),
        ...
        if (pdraw && gg->ui_font && ss->mode == ids::MODE_PLAYING && best_area > 0.0f) {
            ...
            if (best_kind == PK::Item) {
                ...
                SDL_FPoint c = world_to_screen(ggun.pos.x - ggun.size.x * 0.5f,
                                               ggun.pos.y - ggun.size.y * 0.5f);
                float scale = TILE_SIZE * gg->play_cam.zoom;
                r = SDL_Rect{(int)std::floor(c.x), (int)std::floor(c.y),
                             (int)std::ceil(ggun.size.x * scale),
                             (int)std::ceil(ggun.size.y * scale)};
                nm = "gun";
                if (luam) {
                    if (const GunInstance* gi = ss->guns.get(ggun.gun_vid)) {
                        for (auto const& g : luam->guns())
            ...
            if (ss->show_gun_panel) {
                ...
                if (best_kind == PK::Item) {
                    ...
                    int sid = -1;
                    if (luam && inst) {
                        for (auto const& ddf : luam->items())
                            if (ddf.type == inst->def_type) {
                                iname = ddf.name;
                                idesc = ddf.desc;
                                consume = ddf.consume_on_use;
                                if (!ddf.sprite.empty() &&
                                    ddf.sprite.find(':') != std::string::npos)
                                    sid = try_get_sprite_id(ddf.sprite);
                                break;
                            }
                    }
                    if (sid >= 0)
                        if (SDL_Texture* texi = get_texture(sid)) {
                            SDL_Rect dst{tx, ty, 48, 32};
                            SDL_RenderCopy(renderer, texi, nullptr, &dst);
                            ty += 36;
                        }
                    ui_draw_kv_line(tx, ty, lh, "Item", iname);
                    if (!idesc.empty())
                        ui_draw_kv_line(tx, ty, lh, "Desc", idesc);
                    ui_draw_kv_line(tx, ty, lh, "Consumable",
                                    consume ? std::string("Yes") : std::string("No"));
                } else if (best_kind == PK::Gun) {
                    auto const& ggun = ss->ground_guns.data()[best_idx];
                    const GunInstance* gim = ss->guns.get(ggun.gun_vid);
                    const GunDef* gdp = nullptr;
                    if (luam && gim) {
                        for (auto const& gdf : luam->guns())
                            if (gdf.type == gim->def_type) {
                                gdp = &gdf;
                                break;
                            }
                    }
                    if (gdp) {
                        int gun_sid = -1;
                        if (!gdp->sprite.empty())
                            gun_sid = try_get_sprite_id(gdp->sprite);
                        if (gun_sid >= 0)
                            if (SDL_Texture* texg = get_texture(gun_sid)) {
                                SDL_Rect dst{tx, ty, 64, 40};
                                SDL_RenderCopy(renderer, texg, nullptr, &dst);
                                ty += 44;
                            }
                        ui_draw_kv_line(tx, ty, lh, "Gun", gdp->name);
                        ui_draw_kv_line(tx, ty, lh, "Damage",
                                        std::to_string((int)std::lround(gdp->damage)));
                        ui_draw_kv_line(tx, ty, lh, "RPM",
                                        std::to_string((int)std::lround(gdp->rpm)));
                        ui_draw_kv_line(tx, ty, lh, "Deviation", fmt2(gdp->deviation) + " deg");
                        ui_draw_kv_line(tx, ty, lh, "Pellets",
                                        std::to_string(gdp->pellets_per_shot));
                        ui_draw_kv_line(tx, ty, lh, "Recoil", fmt2(gdp->recoil));
                        ui_draw_kv_line(tx, ty, lh, "Control", fmt2(gdp->control));
                        ui_draw_kv_line(
                            tx, ty, lh, "Recoil cap",
                            std::to_string((int)std::lround(gdp->max_recoil_spread_deg)) + " deg");
                        ui_draw_kv_line(
                            tx, ty, lh, "Reload/Eject",
                            std::to_string((int)std::lround(gdp->reload_time * 1000.0f)) + "/" +
                                std::to_string((int)std::lround(gdp->eject_time * 1000.0f)) +
                                " ms");
                        ui_draw_kv_line(tx, ty, lh, "Jam",
                                        std::to_string((int)std::lround(gdp->jam_chance * 100.0f)) +
                                            " %");
                        ui_draw_kv_line(tx, ty, lh, "AR Center",
                                        fmt2(gdp->ar_pos) + " ±" + fmt2(gdp->ar_pos_variance));
                        ui_draw_kv_line(tx, ty, lh, "AR Size",
                                        fmt2(gdp->ar_size) + " ±" + fmt2(gdp->ar_size_variance));
                        if (gim && gim->ammo_type != 0) {
                            if (auto const* ad = luam->find_ammo(gim->ammo_type)) {
                                int asid =
                                    (!ad->sprite.empty()) ? try_get_sprite_id(ad->sprite) : -1;
                                if (asid >= 0)
                                    if (SDL_Texture* tex = get_texture(asid)) {
                                ...
                                if (!ad->desc.empty())
                                    ui_draw_kv_line(tx, ty, lh, "Desc", ad->desc);
                                ...
                                if (ad->range_units > 0.0f) {
                                    ui_draw_kv_line(
                                        tx, ty, lh, "Range",
                                        std::to_string((int)std::lround(ad->range_units)));
                                    ui_draw_kv_line(
                                        tx, ty, lh, "Falloff",
                                        std::to_string((int)std::lround(ad->falloff_start)) + "→" +
                                            std::to_string((int)std::lround(ad->falloff_end)));
                                    ui_draw_kv_line(tx, ty, lh, "Min Mult",
                                                    fmt2(ad->falloff_min_mult));
                                }
                                ui_draw_kv_line(tx, ty, lh, "Speed",
                                                std::to_string((int)std::lround(ad->speed)));
                                ui_draw_kv_line(tx, ty, lh, "Pierce",
                                                std::to_string(ad->pierce_count));
                            }
                        }
                    }
                }
    ...
    for (auto const& proj : ss->projectiles.items)
        if (proj.active) {
            SDL_FPoint c =
                world_to_screen(proj.pos.x - proj.size.x * 0.5f, proj.pos.y - proj.size.y * 0.5f);
            float scale = TILE_SIZE * gg->play_cam.zoom;
            SDL_Rect r{(int)std::floor(c.x), (int)std::floor(c.y),
                ...
                if (gd) {
                    ...
                    glm::vec2 mpos = {
                        ...
                        gg->play_cam.pos.y +
                            (static_cast<float>(ss->mouse_inputs.pos.y) - (float)wh * 0.5f) *
                                inv_scale};
                    float dist = std::sqrt((mpos.x - ppos.x) * (mpos.x - ppos.x) +
                                           (mpos.y - ppos.y) * (mpos.y - ppos.y));
        ...
        if (ss->player_vid) {
            ...
            if (plv && plv->equipped_gun_vid.has_value() && luam) {
                ...
                if (gi) {
                    ...
                    if (gd) {
                        int bar_h = 60, bar_w = 8, gap = 2;
                        int rx = mx + 16;
                        int ry = my - bar_h / 2;
                        if (ss->reload_bar_shake > 0.01f) {
                            static thread_local std::mt19937 rng{std::random_device{}()};
                            std::uniform_real_distribution<float> J(-ss->reload_bar_shake,
                                                                    ss->reload_bar_shake);
                            rx += (int)std::lround(J(rng));
                            ry += (int)std::lround(J(rng));
                            ss->reload_bar_shake *= 0.90f;
                        } else {
                            ss->reload_bar_shake = 0.0f;
                        }
                        float mag_ratio =
                            (gd->mag > 0) ? (float)gi->current_mag / (float)gd->mag : 0.0f;
                        float res_ratio = (gd->ammo_max > 0)
                                              ? (float)gi->ammo_reserve / (float)gd->ammo_max
                                              : 0.0f;
                        ...
                        if (!reloading) {
                            ...
                            if (gi->reload_total_time > 0.0f) {
                                ...
                                int prg_h =
                                    (int)std::lround((double)bar_h * (double)gi->reload_progress);
                                if (prg_h > 0) {
                                    SDL_Rect prog{rx - 2, ry + (bar_h - prg_h), bar_w + 6, prg_h};
                                    if (lockout)
                                        SDL_SetRenderDrawColor(renderer, 110, 110, 120, 220);
                                    else
                        ...
                        if (gg->ui_font) {
                            ...
                            if (txt) {
                                if (SDL_Surface* s =
                                        TTF_RenderUTF8_Blended(gg->ui_font, txt, col)) {
                                    SDL_Texture* t = SDL_CreateTextureFromSurface(renderer, s);
                                    int tw = 0, th = 0;
                                    SDL_QueryTexture(t, nullptr, nullptr, &tw, &th);
    ...
    // Character stats panel (left slide-out)
    {
        ...
        ss->character_panel_slide =
            ss->character_panel_slide +
            (target - ss->character_panel_slide) * (float)std::clamp(6.0 * dt, 0.0, 1.0);
        ...
        if (ss->character_panel_slide > 0.02f && gg->ui_font) {
            ...
            auto draw_line = [&](const char* key, float val, const char* suffix) {
                char buf[64];
                std::snprintf(buf, sizeof(buf), "%s: %.2f%s", key, (double)val, suffix);
                SDL_Color col{220, 220, 220, 255};
                if (SDL_Surface* srf = TTF_RenderUTF8_Blended(gg->ui_font, buf, col)) {
    ...
    if (gg->ui_font && ss->mode == ids::MODE_PLAYING && !ss->show_character_panel) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            bool selected = (pinv ? (pinv->selected_index == (std::size_t)i)
                                  : (ss->inventory.selected_index == (std::size_t)i));
            ...
            const InvEntry* ent =
                pinv ? pinv->get((std::size_t)i) : ss->inventory.get((std::size_t)i);
            if (ent) {
                            ...
                            for (auto const& d : luam->items())
                                if (d.type == inst->def_type) {
                                    ...
                                    if (!d.sprite.empty() &&
                                        ...
                                        sid = try_get_sprite_id(d.sprite);
                                    break;
                                }
                        }
                        draw_icon(sid);
                        ...
                        if (luam) {
                            for (auto const& g : luam->guns())
                                if (g.type == gi->def_type) {
                                    nm = g.name;
                                    if (!g.sprite.empty() &&
                                        g.sprite.find(':') != std::string::npos)
                                        sid = try_get_sprite_id(g.sprite);
                                    break;
                                }
                        }
                        draw_icon(sid);
        ...
        for (auto const& hs : inv_hover_rects) {
            if (mp.x >= hs.r.x && mp.x <= (hs.r.x + hs.r.w) && mp.y >= hs.r.y &&
                mp.y <= (hs.r.y + hs.r.h)) {
        ...
        if (hover_index.has_value()) {
            if (ss->inv_hover_index == (int)*hover_index)
                ss->inv_hover_time += (float)dt;
            else {
                ss->inv_hover_index = (int)*hover_index;
                ss->inv_hover_time = 0.0f;
            }
        } else {
            ss->inv_hover_index = -1;
            ss->inv_hover_time = 0.0f;
        }
        const float HOVER_DELAY = 0.12f;
        // Drag-and-drop
        static bool prev_left = false;
        bool now_left = ss->mouse_inputs.left;
        if (now_left && !prev_left) {
            if (hover_index.has_value()) {
                const Inventory* pinv = (ss->player_vid ? ss->inv_for(*ss->player_vid) : nullptr);
                if ((pinv ? pinv->get(*hover_index) : ss->inventory.get(*hover_index)) != nullptr) {
                    ss->inv_dragging = true;
                    ss->inv_drag_src = (int)*hover_index;
                }
            }
        }
        if (!now_left && prev_left) {
            if (ss->inv_dragging) {
                if (hover_index.has_value()) {
                    ...
                    std::size_t src = (std::size_t)std::max(0, ss->inv_drag_src);
                    if (dst != src) {
                        Inventory* pinv2 =
                            (ss->player_vid ? ss->inv_for(*ss->player_vid) : nullptr);
                        InvEntry* src_e = pinv2 ? pinv2->get_mut(src) : ss->inventory.get_mut(src);
                        InvEntry* dst_e = pinv2 ? pinv2->get_mut(dst) : ss->inventory.get_mut(dst);
                        if (src_e && dst_e) {
                        ...
                        if (pinv2)
                            std::sort(pinv2->entries.begin(), pinv2->entries.end(),
                                      [](auto const& lhs, auto const& rhs) {
                                          return lhs.index < rhs.index;
                                      });
                        else
                            std::sort(ss->inventory.entries.begin(), ss->inventory.entries.end(),
                                      [](auto const& lhs, auto const& rhs) {
                                          return lhs.index < rhs.index;
                                      });
                    }
                }
                ss->inv_dragging = false;
                ss->inv_drag_src = -1;
            }
        }
        prev_left = now_left;
        if (hover_index.has_value() && ss->inv_hover_time >= HOVER_DELAY) {
            ...
            const InvEntry* sel =
                pinv3 ? pinv3->get(*hover_index) : ss->inventory.get(*hover_index);
            if (sel) {
                int panel_w = (int)std::lround(width * 0.32);
                int px = (width - panel_w) / 2;
                int py = (int)std::lround(height * 0.22);
                ...
                if (sel->kind == INV_ITEM) {
                    if (const ItemInstance* inst = ss->items.get(sel->vid)) {
                        ...
                        if (!desc.empty())
                            draw_txt(std::string("Desc: ") + desc, SDL_Color{200, 200, 200, 255});
                    ...
                    if (const GunInstance* gi = ss->guns.get(sel->vid)) {
                        ...
                        if (gdp) {
                            ...
                            if (!gdp->sprite.empty())
                                gun_sid = try_get_sprite_id(gdp->sprite);
                            if (gun_sid >= 0)
                                if (SDL_Texture* tex = get_texture(gun_sid)) {
                                    SDL_Rect dst{tx, ty, 64, 40};
                                    SDL_RenderCopy(renderer, tex, nullptr, &dst);
                                    ty += 44;
                                }
                            draw_txt(std::string("Gun: ") + gdp->name,
                                     SDL_Color{255, 255, 255, 255});
                            draw_txt(std::string("Damage: ") +
                                         std::to_string((int)std::lround(gdp->damage)),
                                     SDL_Color{220, 220, 220, 255});
                            draw_txt(std::string("RPM: ") +
                                         std::to_string((int)std::lround(gdp->rpm)),
                                     SDL_Color{220, 220, 220, 255});
                            draw_txt(std::string("Deviation: ") + std::to_string(gdp->deviation) +
                                         " deg",
                                     SDL_Color{220, 220, 220, 255});
                            draw_txt(std::string("Pellets: ") +
                                         std::to_string(gdp->pellets_per_shot),
                                     SDL_Color{220, 220, 220, 255});
                            draw_txt(std::string("Recoil: ") + std::to_string(gdp->recoil),
                                     SDL_Color{220, 220, 220, 255});
                            draw_txt(std::string("Control: ") + std::to_string(gdp->control),
                                     SDL_Color{220, 220, 220, 255});
                            draw_txt(
                                std::string("Recoil cap: ") +
                                    std::to_string((int)std::lround(gdp->max_recoil_spread_deg)) +
                                    " deg",
                                SDL_Color{220, 220, 220, 255});
                            draw_txt(
                                std::string("Reload/Eject: ") +
                                    std::to_string((int)std::lround(gdp->reload_time * 1000.0f)) +
                                    "/" +
                                    std::to_string((int)std::lround(gdp->eject_time * 1000.0f)) +
                                    " ms",
                                    ...
                                    if (ad->range_units > 0.0f) {
                                        draw_txt(
                                            std::string("Range: ") +
                                                ...
                                                std::string(" (falloff ") +
                                                std::to_string(
                                                    (int)std::lround(ad->falloff_start)) +
                                                std::string("→") +
                                                std::to_string((int)std::lround(ad->falloff_end)) +
                                                std::string(" @ x") +
                                                std::to_string(ad->falloff_min_mult) + ")",
                                            SDL_Color{220, 220, 220, 255});
                                    }
                                    draw_txt(std::string("Speed: ") +
                                                 std::to_string((int)std::lround(ad->speed)) +
                                                 std::string(", Pierce: ") +
                                                 std::to_string(ad->pierce_count),
                                             SDL_Color{220, 220, 220, 255});
    ...
    if (gg->ui_font && ss->mode == ids::MODE_PLAYING && ss->player_vid && luam &&
        ss->show_gun_panel) {
        const Entity* ply = ss->entities.get(*ss->player_vid);
        if (ply && ply->equipped_gun_vid.has_value()) {
            const GunDef* gd = nullptr;
            const GunInstance* gi_inst = ss->guns.get(*ply->equipped_gun_vid);
            if (gi_inst) {
                for (auto const& g : luam->guns())
            ...
            if (gd) {
                ...
                // Icon
                if (!gd->sprite.empty()) {
                    int sid = try_get_sprite_id(gd->sprite);
                    if (sid >= 0)
                        if (SDL_Texture* tex = get_texture(sid)) {
                            SDL_Rect dst{tx, ty, 64, 40};
                            SDL_RenderCopy(renderer, tex, nullptr, &dst);
                ...
                ui_draw_kv_line(tx, ty, lh, "Recoil cap",
                                std::to_string((int)std::lround(gd->max_recoil_spread_deg)) +
                                    " deg");
                ui_draw_kv_line(tx, ty, lh, "Reload",
                                std::to_string((int)std::lround(gd->reload_time * 1000.0f)) +
                                    " ms");
                ui_draw_kv_line(tx, ty, lh, "Eject",
                                std::to_string((int)std::lround(gd->eject_time * 1000.0f)) + " ms");
                ui_draw_kv_line(tx, ty, lh, "Jam",
                                std::to_string((int)std::lround(gd->jam_chance * 100.0f)) + " %");
                ui_draw_kv_line(tx, ty, lh, "AR Center",
                                fmt2(gd->ar_pos) + " ±" + fmt2(gd->ar_pos_variance));
                ui_draw_kv_line(tx, ty, lh, "AR Size",
                                fmt2(gd->ar_size) + " ±" + fmt2(gd->ar_size_variance));
                if (gd->rpm > 0.0f || gd->shot_interval > 0.0f) {
                    float shot_dt = gd->shot_interval > 0.0f ? gd->shot_interval
                                                             : (60.0f / std::max(1.0f, gd->rpm));
                    int ms = (int)std::lround(shot_dt * 1000.0f);
                    draw_txt(std::string("Shot Time: ") + std::to_string(ms) + " ms",
                             SDL_Color{220, 220, 220, 255});
                }
                if (gd->fire_mode == std::string("burst") &&
                    (gd->burst_rpm > 0.0f || gd->burst_interval > 0.0f)) {
                    draw_txt(std::string("Burst RPM: ") +
                                 std::to_string((int)std::lround(gd->burst_rpm)),
                             SDL_Color{220, 220, 220, 255});
                    float bdt = gd->burst_interval > 0.0f
                                    ? gd->burst_interval
                                    : (gd->burst_rpm > 0.0f ? 60.0f / gd->burst_rpm : 0.0f);
                    if (bdt > 0.0f) {
                ...
                // Fire mode label
                {
                    ...
                    if (gd->fire_mode == "single")
                        ...
                        fm_label = "Burst";
                    draw_txt(std::string("Mode: ") + fm_label, SDL_Color{220, 220, 220, 255});
                }
                // Current mag/reserve
                if (gi_inst) {
                    ...
                    if (gi_inst->ammo_type != 0) {
                        if (auto const* ad = luam->find_ammo(gi_inst->ammo_type)) {
                            ...
                            ui_draw_kv_line(tx, ty, lh, "Shield", fmt2(ad->shield_mult));
                            if (ad->range_units > 0.0f) {
                                ui_draw_kv_line(tx, ty, lh, "Range",
                                                std::to_string((int)std::lround(ad->range_units)));
                                ui_draw_kv_line(
                                    tx, ty, lh, "Falloff",
                                    std::to_string((int)std::lround(ad->falloff_start)) + "→" +
    ...
    if (gg->ui_font && ss->mode == ids::MODE_PLAYING && ss->player_vid) {
        ...
        if (p) {
            ...
            if (p->stats.shield_max > 0.0f) {
                float sratio =
                    (p->stats.shield_max > 0.0f) ? (p->shield / p->stats.shield_max) : 0.0f;
                ...
                draw_num(std::to_string((int)std::lround(p->stats.shield_max)), gx + group_w + 6,
                         gy, white);
            }
            // Plates
            ...
            if (ss->dash_max > 0) {
                ...
                if (ss->dash_stocks < ss->dash_max) {
                    double pratio = std::clamp(
                        (double)(ss->dash_refill_timer / DASH_COOLDOWN_SECONDS), 0.0, 1.0);
    ...
    if (ss->mode == ids::MODE_PLAYING && ss->exit_countdown >= 0.0f) {
        ...
        if (gg->ui_font) {
            ...
            char txt[16];
            float secs = std::max(0.0f, ss->exit_countdown);
            std::snprintf(txt, sizeof(txt), "%.1f", (double)secs);
            SDL_Color color{255, 255, 255, 255};
            if (SDL_Surface* surf = TTF_RenderUTF8_Blended(gg->ui_font, txt, color)) {
    ...
    if (ss->mode == ids::MODE_SCORE_REVIEW) {
        ...
        if (gg->ui_font) {
            ...
            if (ss->score_ready_timer <= 0.0f) {
                ...
                if (SDL_Surface* ps = TTF_RenderUTF8_Blended(
                        gg->ui_font, "Press SPACE or CLICK to continue", pc)) {
                    SDL_Texture* ptex = SDL_CreateTextureFromSurface(renderer, ps);
                    int prompt_w = 0, prompt_h = 0;
                    SDL_QueryTexture(ptex, nullptr, nullptr, &prompt_w, &prompt_h);
                    SDL_Rect pd{width / 2 - prompt_w / 2, height - prompt_h - 20, prompt_w,
                                prompt_h};
        ...
        if (ss->score_ready_timer <= 0.0f) {
            ...
            if (ss->review_next_stat_timer <= 0.0f &&
                ss->review_revealed < ss->review_stats.size()) {
                ss->review_next_stat_timer = 0.2f;
                ss->review_revealed += 1;
                if (aa)
                    play_sound("base:small_shoot");
            }
            ...
            for (std::size_t i = 0; i < ss->review_revealed && i < ss->review_stats.size(); ++i) {
                ...
                if (rs.header) {
                    ...
                    if (std::fabs(rs.target - std::round(rs.target)) < 0.001)
                        std::snprintf(buf, sizeof(buf), "%s: %d", rs.label.c_str(),
                                      (int)std::lround(rs.value));
                    else
    ...
    if (ss->mode == ids::MODE_NEXT_STAGE) {
        ...
        if (gg->ui_font) {
            SDL_Color titlec{240, 220, 80, 255};
            if (SDL_Surface* ts = TTF_RenderUTF8_Blended(gg->ui_font, "Next Area", titlec)) {
                SDL_Texture* tt = SDL_CreateTextureFromSurface(renderer, ts);
                int tw = 0, th = 0;
                SDL_QueryTexture(tt, nullptr, nullptr, &tw, &th);
