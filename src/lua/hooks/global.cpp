...
void LuaManager::call_on_dash(Entity& player) {
    ...
    if (hooks_->global.on_dash.valid()) {
        auto r = hooks_->global.on_dash();
        if (!r.valid()) {
            sol::error e = r;
            std::fprintf(stderr, "[lua] on_dash error: %s\n", e.what());
        }
    } else {
...
void LuaManager::call_on_step(Entity* player) {
    LuaCtxGuard _ctx(ss, player);
    if (hooks_->global.on_step.valid()) {
        auto r = hooks_->global.on_step();
        if (!r.valid()) {
            sol::error e = r;
...
void LuaManager::call_on_active_reload(Entity& player) {
    LuaCtxGuard _ctx(ss, &player);
    if (hooks_->global.on_active_reload.valid()) {
        auto r = hooks_->global.on_active_reload();
        if (!r.valid()) {
            sol::error e = r;
            std::fprintf(stderr, "[lua] on_active_reload error: %s\n", e.what());
        }
    } else {
...
void LuaManager::call_on_failed_active_reload(Entity& player) {
    LuaCtxGuard _ctx(ss, &player);
    if (hooks_->global.on_failed_active_reload.valid()) {
        auto r = hooks_->global.on_failed_active_reload();
        if (!r.valid()) {
            sol::error e = r;
...
void LuaManager::call_on_tried_after_failed_ar(Entity& player) {
    LuaCtxGuard _ctx(ss, &player);
    if (hooks_->global.on_tried_after_failed_ar.valid()) {
        auto r = hooks_->global.on_tried_after_failed_ar();
        if (!r.valid()) {
            sol::error e = r;
            std::fprintf(stderr, "[lua] on_tried_to_active_reload_after_failing error: %s\n",
                         e.what());
        }
    } else {
...
void LuaManager::call_on_eject(Entity& player) {
    LuaCtxGuard _ctx(ss, &player);
    if (hooks_->global.on_eject.valid()) {
        auto r = hooks_->global.on_eject();
        if (!r.valid()) {
            sol::error e = r;
...
void LuaManager::call_on_reload_start(Entity& player) {
    LuaCtxGuard _ctx(ss, &player);
    if (hooks_->global.on_reload_start.valid()) {
        auto r = hooks_->global.on_reload_start();
        if (!r.valid()) {
            sol::error e = r;
...
void LuaManager::call_on_reload_finish(Entity& player) {
    LuaCtxGuard _ctx(ss, &player);
    if (hooks_->global.on_reload_finish.valid()) {
        auto r = hooks_->global.on_reload_finish();
        if (!r.valid()) {
            sol::error e = r;
