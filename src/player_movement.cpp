    ...
    for (auto& e : ss->entities.data()) {
        ...
        if (e.type_ == ids::ET_PLAYER) {
            ...
            if (ss->dash_stocks < ss->dash_max) {
                ...
                while (ss->dash_refill_timer >= DASH_COOLDOWN_SECONDS &&
                       ss->dash_stocks < ss->dash_max) {
            ...
            // Movement inaccuracy accumulator
            {
                ...
                if (factor > 0.01f) {
                    e.move_spread_deg = std::min(
                        e.stats.move_spread_max_deg,
                        e.move_spread_deg +
                            e.stats.move_spread_inc_rate_deg_per_sec_at_base * factor * TIMESTEP);
                } else {
                    e.move_spread_deg = std::max(
                        0.0f, e.move_spread_deg - e.stats.move_spread_decay_deg_per_sec * TIMESTEP);
                }
            }
        ...
        for (int s = 0; s < steps; ++s) {
            ...
            float next_x = e.pos.x + step_dpos.x;
            {
                ...
                int minx = (int)floorf(tl.x), miny = (int)floorf(tl.y), maxx = (int)floorf(br.x),
                    maxy = (int)floorf(br.y);
                bool blocked = false;
                for (int ty = miny; ty <= maxy && !blocked; ++ty)
                    for (int tx = minx; tx <= maxx; ++tx) {
                        if (!ss->stage.in_bounds(tx, ty)) {
                            blocked = true;
                            break;
                        }
                        if (ss->stage.at(tx, ty).blocks_entities()) {
                            blocked = true;
                            break;
                        }
                    }
