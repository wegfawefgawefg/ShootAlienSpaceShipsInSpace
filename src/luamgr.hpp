...
#ifndef SHOOTALIENSPACESHIPSINSPACE_LUAMGR_HPP
#define SHOOTALIENSPACESHIPSINSPACE_LUAMGR_HPP
...
struct LuaManager {
  public:
    ...
    const std::vector<AmmoDef>& ammo() const {
        return ammo_;
    }
    ...
    void add_ammo(const AmmoDef& d) {
        ammo_.push_back(d);
    }
    void add_crate(const CrateDef& d) {
        crates_.push_back(d);
    }
    void add_entity_type(const EntityTypeDef& d) {
        entity_types_.push_back(d);
    }
