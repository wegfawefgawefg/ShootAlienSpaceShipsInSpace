...
void update_shields_and_reload_progress() {
    for (auto& e : ss->entities.data()) {
        ...
        if (e.stats.shield_max > 0.0f && e.time_since_damage >= 3.0f) {
            ...
            if (luam && e.def_type) {
                if (prev_ratio < 1.0f && ratio >= 1.0f)
                    luam->call_entity_on_shield_full(e.def_type, e);
                if (prev_ratio >= 0.5f && ratio < 0.5f)
                    luam->call_entity_on_shield_under_50(e.def_type, e);
                if (prev_ratio >= 0.25f && ratio < 0.25f)
                    luam->call_entity_on_shield_under_25(e.def_type, e);
            }
...
void update_reload_active() {
    if (!ss || ss->mode != ids::MODE_PLAYING || !ss->player_vid)
        return;
        ...
        if (gim) {
            ...
            if (gim->jammed) {
                ...
                if (gim->reloading) {
                                ...
                                for (const auto& entry : inv->entries) {
                                    if (entry.kind == INV_ITEM)
                                        if (const ItemInstance* inst = ss->items.get(entry.vid))
                                            luam->call_item_on_active_reload(inst->def_type, *plm);
                                }
                        }
                        ...
                        if (luam) {
                            ...
                            if (auto* inv =
                                ...
                                for (const auto& entry : inv->entries) {
                                    if (entry.kind == INV_ITEM)
                                        if (const ItemInstance* inst = ss->items.get(entry.vid))
                                            luam->call_item_on_failed_active_reload(inst->def_type,
                                                                                    *plm);
                                }
                        }
                        ...
                        if (luam) {
                            ...
                            if (auto* inv =
                                ...
                                for (const auto& entry : inv->entries) {
                                    if (entry.kind == INV_ITEM)
                                        if (const ItemInstance* inst = ss->items.get(entry.vid))
                                            luam->call_item_on_tried_after_failed_ar(inst->def_type,
                                                                                     *plm);
                                }
                        }
                    }
                    ...
                    if (dropped > 0)
                        ss->alerts.push_back(
                            {std::string("Dropped ") + std::to_string(dropped) + " bullets", 0.0f,
                             1.0f, false});
                    gim->current_mag = 0;
                    gim->reloading = true;
                    if (plm->def_type && luam)
                        luam->call_entity_on_reload_start(plm->def_type, *plm);
                    if (ss->player_vid)
                        if (auto* pm = ss->metrics_for(*ss->player_vid))
                            pm->reloads += 1;
                    gim->reload_progress = 0.0f;
                    ...
                    std::uniform_real_distribution<float> Upos(-gd->ar_pos_variance,
                                                               gd->ar_pos_variance);
                    std::uniform_real_distribution<float> Usize(-gd->ar_size_variance,
                                                                gd->ar_size_variance);
                    ...
                    if (aa)
                        play_sound(gd->sound_reload.empty() ? "base:reload" : gd->sound_reload);
                } else {
                    ss->alerts.push_back({std::string("NO AMMO"), 0.0f, 1.5f, false});
                    if (plm->def_type && luam)
                        luam->call_entity_on_out_of_ammo(plm->def_type, *plm);
                }
            }
        }
    }
...
void update_trigger_and_fire() {
    ...
    if (ss->mode == ids::MODE_PLAYING && ss->player_vid) {
        ...
        if (plm && plm->equipped_gun_vid.has_value()) {
            ...
            if (gimq) {
                ...
                if (gdq)
                    gimq->spread_recoil_deg =
                        std::max(0.0f, gimq->spread_recoil_deg - gdq->control * TIMESTEP);
                if (fire_mode == "auto")
    ...
    if (!(ss->mode == ids::MODE_PLAYING && ss->input_lockout_timer == 0.0f && fire_request &&
          can_fire))
        return;
    ...
    if (ss->player_vid) {
        ...
        if (plm && plm->equipped_gun_vid.has_value()) {
            ...
            if (gd && gi) {
                ...
                if (fired) {
                    float acc =
                        std::max(0.1f, ss->entities.get(*ss->player_vid)->stats.accuracy / 100.0f);
                ...
                if (fired) {
                    ...
                    if (U(rng) < jc) {
                        ...
                        if (luam) {
                            ...
                            if (plm->def_type)
                                luam->call_entity_on_gun_jam(plm->def_type, *plm);
                        }
                        if (aa)
                            play_sound(gd->sound_jam.empty() ? "base:ui_cant" : gd->sound_jam);
                ...
                if (fired && ss->player_vid)
                    if (auto* pm = ss->metrics_for(*ss->player_vid))
                        pm->shots_fired += 1;
                if (fired && gd && gim)
                    gim->spread_recoil_deg =
                        std::min(gim->spread_recoil_deg + gd->recoil, gd->max_recoil_spread_deg);
            }
        }
    }
    if (fired) {
        ...
        for (int i = 0; i < pellets; ++i) {
            ...
            auto* pr =
                ss ? ss->projectiles.spawn(sp, pdir * proj_speed, proj_size, proj_steps, proj_type)
                   : nullptr;
            if (pr && ss->player_vid)
        ...
        if (ss->player_vid) {
            ...
            if (plm && plm->equipped_gun_vid.has_value()) {
                ...
                if (aa)
                    play_sound((gd && !gd->sound_fire.empty()) ? gd->sound_fire
                                                               : "base:small_shoot");
            } else {
                if (aa)
...
void update_unjam() {
    if (!ss || !ss->player_vid || ss->mode != ids::MODE_PLAYING)
        return;
    auto* plm = ss->entities.get_mut(*ss->player_vid);
    if (!plm || !plm->equipped_gun_vid.has_value())
        return;
    ...
    if (gim->unjam_progress >= 1.0f) {
        ...
        if (gd) {
            if (gim->ammo_reserve > 0) {
                ...
                if (dropped > 0)
                    ss->alerts.push_back(
                        {std::string("Dropped ") + std::to_string(dropped) + " bullets", 0.0f, 1.5f,
                         false});
                // start active reload
                gim->current_mag = 0;
                gim->reloading = true;
                ...
                std::uniform_real_distribution<float> Upos2(-gd->ar_pos_variance,
                                                            gd->ar_pos_variance);
                std::uniform_real_distribution<float> Usize2(-gd->ar_size_variance,
                                                             gd->ar_size_variance);
