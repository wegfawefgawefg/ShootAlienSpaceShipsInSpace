...
static inline NavNode nav_main_for_id(int id) {
    switch (id) {
    case 100:
        return NavNode{-1, 101, 100, 100};
    case 101:
        return NavNode{100, 102, 101, 101};
    case 102:
        return NavNode{101, -1, 102, 102};
    default:
...
static inline NavNode nav_settings_for_id(int id) {
    switch (id) {
    case 201:
        return NavNode{-1, 200, 201, 201}; // Video (top)
    case 200:
        return NavNode{201, 202, 200, 200}; // Audio
    case 202:
        return NavNode{200, 204, 202, 202}; // Controls
    case 204:
        return NavNode{202, 205, 204, 204}; // Binds
    case 205:
        return NavNode{204, 203, 205, 205}; // Players
    case 203:
        return NavNode{205, 299, 203, 203}; // Other
    case 299:
        return NavNode{203, -1, 299, 299}; // Back
    default:
        return NavNode{-1, -1, -1, -1};
    }
}
...
static inline NavNode nav_audio_for_id(int id) {
    switch (id) {
    case 300:
        return NavNode{-1, 301, 300, 300};
    case 301:
        return NavNode{300, 302, 301, 301};
    case 302:
        return NavNode{301, 399, 302, 302};
    case 399:
        return NavNode{302, -1, 399, 399};
    default:
        return NavNode{-1, -1, -1, -1};
    }
}
static inline NavNode nav_video_for_id(int id) {
    switch (id) {
    case 400:
        return NavNode{-1, 405, 400, 400};
    case 405:
        return NavNode{400, 401, 405, 405};
    case 401:
        return NavNode{405, 402, 401, 401};
    case 402:
        return NavNode{401, 403, 402, 402};
    case 403:
        return NavNode{402, 404, 403, 403};
    case 404:
        return NavNode{403, 499, 404, 404};
    case 499:
        return NavNode{404, -1, 499, 498}; // Back ↔ Apply horizontally
    case 498:
        return NavNode{404, -1, 499, 498};
    default:
        return NavNode{-1, -1, -1, -1};
    }
}
static inline NavNode nav_controls_for_id(int id) {
    switch (id) {
    case 500:
        return NavNode{-1, 501, 500, 500};
    case 501:
        return NavNode{500, 502, 501, 501};
    case 502:
        return NavNode{501, 505, 502, 502};
    case 505:
        return NavNode{502, 507, 505, 505};
    case 507:
        return NavNode{505, 506, 507, 507};
    case 506:
        return NavNode{507, 599, 506, 506};
    case 599:
        return NavNode{506, -1, 599, 599};
    default:
        return NavNode{-1, -1, -1, -1};
    }
}
static inline NavNode nav_binds_for_id(int id) {
    ...
    switch (id) {
    case 799:
        return NavNode{704, -1, 799, 791};
    case 791:
        return NavNode{704, -1, 799, 792};
    case 792:
        return NavNode{704, -1, 791, 793};
    case 793:
        return NavNode{704, -1, 792, 794};
    case 794:
        return NavNode{704, -1, 793, 794};
    default:
        return NavNode{-1, -1, -1, -1};
    }
}
...
static inline NavNode nav_other_for_id(int id) {
    switch (id) {
    case 800:
        return NavNode{-1, 801, 800, 800};
    case 801:
        return NavNode{800, 899, 801, 801};
    case 899:
        return NavNode{801, -1, 899, 899};
    default:
        return NavNode{-1, -1, -1, -1};
    }
}
...
inline void ensure_focus_default(const std::vector<ButtonDesc>& btns) {
    if (ss->menu.focus_id >= 0)
        return;
    for (auto const& b : btns)
        if (b.enabled) {
            ss->menu.focus_id = b.id;
            break;
        }
}
...
std::vector<RectNdc> layout_vlist(RectNdc top_left, float item_w, float item_h, float vgap,
                                  int count) {
...
std::vector<ButtonDesc> build_menu_buttons(int width, int height) {
    ...
    if (ss->menu.page == MAIN) {
        ...
        buttons.push_back(ButtonDesc{299, RectNdc{0.00f, 0.82f, 0.20f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
    } else if (ss->menu.page == AUDIO) {
        auto rects = layout_vlist(RectNdc{0.10f, 0.20f, 0.0f, 0.0f}, 0.60f, item_h, vgap, 3);
        buttons.push_back(ButtonDesc{300, rects[0], "Master Volume", ButtonKind::Slider,
                                     ss->menu.vol_master, true});
        buttons.push_back(ButtonDesc{301, rects[1], "Music Volume", ButtonKind::Slider,
                                     ss->menu.vol_music, true});
        buttons.push_back(
            ButtonDesc{302, rects[2], "SFX Volume", ButtonKind::Slider, ss->menu.vol_sfx, true});
        // Footer buttons (5% from left/bottom)
        buttons.push_back(ButtonDesc{399, RectNdc{0.00f, 0.82f, 0.20f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
    } else if (ss->menu.page == VIDEO) {
        auto rects = layout_vlist(RectNdc{0.10f, 0.20f, 0.0f, 0.0f}, 0.70f, item_h, vgap, 6);
        buttons.push_back(ButtonDesc{400, rects[0], "Resolution", ButtonKind::OptionCycle,
                                     static_cast<float>(ss->menu.video_res_index), true});
        bool windowed = (std::clamp(ss->menu.window_mode_index, 0, 2) == 0);
        buttons.push_back(ButtonDesc{405, rects[1], "Window Size", ButtonKind::OptionCycle,
                                     static_cast<float>(ss->menu.window_size_index), windowed});
        buttons.push_back(ButtonDesc{401, rects[2], "Window Mode", ButtonKind::OptionCycle,
                                     static_cast<float>(ss->menu.window_mode_index), true});
        buttons.push_back(ButtonDesc{402, rects[3], "VSync", ButtonKind::Toggle,
                                     ss->menu.vsync ? 1.0f : 0.0f, true});
        buttons.push_back(ButtonDesc{403, rects[4], "Frame Rate Limit", ButtonKind::OptionCycle,
                                     static_cast<float>(ss->menu.frame_limit_index), true});
        buttons.push_back(
            ButtonDesc{404, rects[5], "UI Scale", ButtonKind::Slider, ss->menu.ui_scale, true});
        ...
        if (gg) {
            ...
            bool res_diff =
                (gg->dims.x != (unsigned)kResW[idx] || gg->dims.y != (unsigned)kResH[idx]);
            Uint32 flags = SDL_GetWindowFlags(gg->window);
            int curr_mode = (flags & SDL_WINDOW_FULLSCREEN)
                                ? 2
                                : ((flags & SDL_WINDOW_FULLSCREEN_DESKTOP) ? 1 : 0);
            bool mode_diff = (std::clamp(ss->menu.window_mode_index, 0, 2) != curr_mode);
            apply_enabled = res_diff || mode_diff;
        }
        buttons.push_back(ButtonDesc{499, RectNdc{0.00f, 0.82f, 0.20f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
        buttons.push_back(ButtonDesc{498, RectNdc{0.70f, 0.82f, 0.20f, 0.08f}, "Apply",
                                     ButtonKind::Button, 0.0f, apply_enabled});
    } else if (ss->menu.page == CONTROLS) {
        auto rects = layout_vlist(RectNdc{0.10f, 0.16f, 0.0f, 0.0f}, 0.70f, item_h, vgap, 4);
        buttons.push_back(ButtonDesc{500, rects[0], "Screen Shake", ButtonKind::Slider,
                                     ss->menu.screen_shake, true});
        buttons.push_back(ButtonDesc{501, rects[1], "Mouse Sensitivity", ButtonKind::Slider,
                                     ss->menu.mouse_sens, true});
        buttons.push_back(ButtonDesc{502, rects[2], "Controller Sensitivity", ButtonKind::Slider,
                                     ss->menu.controller_sens, true});
        buttons.push_back(ButtonDesc{505, rects[3], "Vibration Enabled", ButtonKind::Toggle,
                                     ss->menu.vibration_enabled ? 1.0f : 0.0f, true});
        // Add vibration magnitude slider row
        auto ctrl_more = layout_vlist(RectNdc{0.10f, 0.16f + 4 * (item_h + vgap), 0.0f, 0.0f},
                                      0.70f, item_h, vgap, 1);
        buttons.push_back(ButtonDesc{507, ctrl_more[0], "Vibration Intensity", ButtonKind::Slider,
                                     ss->menu.vibration_magnitude, ss->menu.vibration_enabled});
        buttons.push_back(ButtonDesc{599, RectNdc{0.00f, 0.82f, 0.20f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
        ...
        auto eq_binds = [](const InputBindings& a, const InputBindings& b) {
            return a.left == b.left && a.right == b.right && a.up == b.up && a.down == b.down &&
                   a.use_left == b.use_left && a.use_right == b.use_right && a.use_up == b.use_up &&
                   a.use_down == b.use_down && a.use_center == b.use_center &&
                   a.pick_up == b.pick_up && a.drop == b.drop && a.reload == b.reload &&
                   a.dash == b.dash;
        };
        bool modified = !eq_binds(ss->input_binds, ss->menu.binds_snapshot);
        buttons.push_back(ButtonDesc{799, RectNdc{0.00f, 0.82f, 0.18f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
        buttons.push_back(ButtonDesc{791, RectNdc{0.20f, 0.82f, 0.22f, 0.08f}, "Reset to Defaults",
                                     ButtonKind::Button, 0.0f, true});
        buttons.push_back(ButtonDesc{792, RectNdc{0.44f, 0.82f, 0.18f, 0.08f}, "Load Preset",
                                     ButtonKind::Button, 0.0f, true});
        buttons.push_back(ButtonDesc{793, RectNdc{0.64f, 0.82f, 0.18f, 0.08f}, "New Preset",
                                     ButtonKind::Button, 0.0f, true});
        buttons.push_back(ButtonDesc{794, RectNdc{0.84f, 0.82f, 0.16f, 0.08f}, "Undo Changes",
                                     ButtonKind::Button, 0.0f, modified});
        ...
        buttons.push_back(ButtonDesc{799, RectNdc{0.00f, 0.82f, 0.20f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
    } else if (ss->menu.page == PLAYERS) {
        // Ensure players_presets size
        if ((int)ss->menu.players_presets.size() < ss->menu.players_count)
            ss->menu.players_presets.resize(static_cast<std::size_t>(ss->menu.players_count),
                                            ss->menu.binds_current_preset.empty()
                                                ? std::string("Default")
                                                : ss->menu.binds_current_preset);
        int per_page = 5;
        int start = ss->menu.players_page * per_page;
        int rows = std::min(per_page, ss->menu.players_count - start);
        auto rows_rect =
            layout_vlist(RectNdc{0.10f, 0.20f, 0.0f, 0.0f}, 0.70f, item_h, vgap, std::max(0, rows));
        for (int i = 0; i < rows; ++i) {
            buttons.push_back(ButtonDesc{900 + i, rows_rect[static_cast<std::size_t>(i)], "Player",
                                         ButtonKind::OptionCycle, 0.0f, true});
        }
        // Footer: Add/Remove/Back
        buttons.push_back(ButtonDesc{981, RectNdc{0.22f, 0.82f, 0.20f, 0.08f}, "Remove Player",
                                     ButtonKind::Button, 0.0f, ss->menu.players_count > 1});
        buttons.push_back(ButtonDesc{980, RectNdc{0.44f, 0.82f, 0.20f, 0.08f}, "Add Player",
                                     ButtonKind::Button, 0.0f, true});
        buttons.push_back(ButtonDesc{999, RectNdc{0.00f, 0.82f, 0.20f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
    } else if (ss->menu.page == OTHER) {
        auto rects = layout_vlist(RectNdc{0.10f, 0.20f, 0.0f, 0.0f}, 0.60f, item_h, vgap, 0);
        buttons.push_back(ButtonDesc{899, RectNdc{0.00f, 0.82f, 0.20f, 0.08f}, "Back",
                                     ButtonKind::Button, 0.0f, true});
    }
    return buttons;
}
...
void render_menu(int width, int height) {
    ...
    auto draw_text_hdr = [&](const char* s, int x, int y) {
        ...
        if (SDL_Surface* ts = TTF_RenderUTF8_Blended(gg->ui_font, s, tc)) {
            ...
            SDL_Rect td{x, y, sw, sh};
            SDL_RenderCopy(r, tt, nullptr, &td);
    ...
    if (ss->menu.page == VIDEO) {
        ...
        char buf[64];
        std::snprintf(buf, sizeof(buf), "Page %d/%d", ss->menu.page_index + 1, 1);
        int x = width - 200;
        int y = hdr_y;
        ...
        if (total_pages > 1) {
            ...
            SDL_RenderFillRect(r, &prevb);
            SDL_SetRenderDrawColor(r, 180, 180, 190, 255);
            SDL_RenderDrawRect(r, &prevb);
            SDL_SetRenderDrawColor(r, 40, 45, 55, 255);
            SDL_RenderFillRect(r, &nextb);
            SDL_SetRenderDrawColor(r, 180, 180, 190, 255);
            SDL_RenderDrawRect(r, &nextb);
            auto draw_small = [&](const char* s, SDL_Rect rc) {
                if (!gg->ui_font)
                    return;
                SDL_Color c{220, 220, 230, 255};
                if (SDL_Surface* srf = TTF_RenderUTF8_Blended(gg->ui_font, s, c)) {
                    SDL_Texture* tex = SDL_CreateTextureFromSurface(r, srf);
                    int tw = 0, th = 0;
                    SDL_QueryTexture(tex, nullptr, nullptr, &tw, &th);
                    SDL_Rect d{rc.x + (rc.w - tw) / 2, rc.y + (rc.h - th) / 2, tw, th};
        ...
        char buf[64];
        std::snprintf(buf, sizeof(buf), "Page %d/%d", ss->menu.binds_keys_page + 1, total_pages);
        int x = width - 380;
        int y = hdr_y;
        draw_text_hdr(buf, x, y);
        if (total_pages > 1) {
        ...
        char buf[64];
        std::snprintf(buf, sizeof(buf), "Page %d/%d", ss->menu.binds_list_page + 1, total_pages);
        int x = width - 380;
        int y = hdr_y;
        draw_text_hdr(buf, x, y);
        if (total_pages > 1) {
    ...
    auto draw_label_value = [&](const SDL_Rect& rr, const std::string& label,
        ...
        if (!value.empty() && gg->ui_font) {
            if (SDL_Surface* vs = TTF_RenderUTF8_Blended(gg->ui_font, value.c_str(), vc)) {
                ...
                int tw = 0, th = 0;
                SDL_QueryTexture(vt, nullptr, nullptr, &tw, &th);
    ...
    auto draw_slider = [&](const SDL_Rect& rr, float t) {
        ...
        SDL_Rect fg{bx, by, fw, bh};
        SDL_RenderFillRect(r, &fg);
        int thw = 8, thh = 14;
        int tx = bx + fw - thw / 2;
        int ty = by - (thh - bh) / 2;
        SDL_SetRenderDrawColor(r, 200, 200, 210, 255);
        SDL_Rect thumb{tx, ty, thw, thh};
    ...
    if (ss->menu.page == BINDS) {
        ...
        std::string preset = ss->menu.binds_current_preset.empty() ? std::string("Default")
                                                                   : ss->menu.binds_current_preset;
        bool modified = !eq_binds(ss->input_binds, ss->menu.binds_snapshot);
    ...
    if (ss->menu.page == BINDS || ss->menu.page == BINDS_LOAD) {
        const char* hint = (ss->menu.last_input_source == 2) ? "Prev: LB  Next: RB  Back: B"
                                                             : "Prev: Q  Next: E  Back: Esc";
    ...
    for (auto const& b : buttons) {
        ...
        SDL_Rect rr{(int)std::floor(pr.x), (int)std::floor(pr.y), (int)std::ceil(pr.w),
                    (int)std::ceil(pr.h)};
        ...
        if (b.kind == ButtonKind::Button) {
            if (ss->menu.page == BINDS && b.id >= 700 && b.id <= 704) {
                static const BindAction order[] = {
                    BA_LEFT,      BA_RIGHT,  BA_UP,       BA_DOWN,       BA_USE_LEFT,
                    BA_USE_RIGHT, BA_USE_UP, BA_USE_DOWN, BA_USE_CENTER, BA_PICK_UP,
                    BA_DROP,      BA_RELOAD, BA_DASH};
                int per_page = 5;
            ...
            int lx = rr.x + 12;
            int ly = rr.y + rr.h / 2 - 8;
            draw_text(b.label.c_str(), lx, ly, lc);
            ...
            SDL_SetRenderDrawColor(r, 60, 60, 70, 255);
            SDL_RenderFillRect(r, &leftA);
            SDL_RenderFillRect(r, &rightA);
            SDL_SetRenderDrawColor(r, 180, 180, 190, 255);
            SDL_RenderDrawRect(r, &leftA);
            SDL_RenderDrawRect(r, &rightA);
...
void step_menu_logic(int width, int height) {
    ...
    // If capturing a bind, ignore all menu navigation and clicks until capture completes (Esc
    // cancels in input.cpp)
    if (ss->menu.capture_action_id >= 0) {
    ...
    auto nav = [&](int id) -> NavNode {
        switch (ss->menu.page) {
        case MAIN:
            return nav_main_for_id(id);
        case SETTINGS:
            return nav_settings_for_id(id);
        case AUDIO:
            return nav_audio_for_id(id);
        case VIDEO:
            return nav_video_for_id(id);
        case CONTROLS:
            return nav_controls_for_id(id);
        case BINDS:
            return nav_binds_for_id(id);
        case BINDS_LOAD:
            return nav_binds_list_for_id(id);
        case OTHER:
            return nav_other_for_id(id);
        default:
            return NavNode{-1, -1, -1, -1};
        }
    };
        ...
        if (inside) {
            ...
            if (click) {
                ...
                if (b.id == 100) {
                    ...
                    if (aa)
                        play_sound("base:ui_confirm");
                } else if (b.id == 399 || b.id == 499 || b.id == 599 || b.id == 799 ||
                           b.id == 899) {
                    ss->menu.page = SETTINGS;
                    ss->menu.focus_id = -1;
                    ss->menu.ignore_mouse_until_release = true;
                    ...
                    if (b.id == 402)
                        ss->menu.vsync = !ss->menu.vsync;
                    else if (b.id == 503)
                        ss->menu.invert_x = !ss->menu.invert_x;
                    else if (b.id == 504)
                        ss->menu.invert_y = !ss->menu.invert_y;
                    else if (b.id == 505)
                        ss->menu.vibration_enabled = !ss->menu.vibration_enabled;
                } else if (b.kind == ButtonKind::OptionCycle) {
                    if (!ss->menu.ignore_mouse_until_release) {
                        // Determine arrow hit boxes (larger)
                        SDL_Rect rr{(int)std::floor(pr.x), (int)std::floor(pr.y),
                                    (int)std::ceil(pr.w), (int)std::ceil(pr.h)};
                        int aw = std::min(36, rr.w / 6);
                        int ah = std::min(28, rr.h - 8);
                        SDL_Rect leftA{rr.x + rr.w - (aw * 2 + 8), rr.y + rr.h / 2 - ah / 2, aw,
                                       ah};
                        SDL_Rect rightA{rr.x + rr.w - aw, rr.y + rr.h / 2 - ah / 2, aw, ah};
                        bool inLeft = (ss->mouse_inputs.pos.x >= leftA.x &&
                                       ss->mouse_inputs.pos.x <= leftA.x + leftA.w &&
                                       ss->mouse_inputs.pos.y >= leftA.y &&
                                       ss->mouse_inputs.pos.y <= leftA.y + leftA.h);
                        bool inRight = (ss->mouse_inputs.pos.x >= rightA.x &&
                                        ss->mouse_inputs.pos.x <= rightA.x + rightA.w &&
                                        ss->mouse_inputs.pos.y >= rightA.y &&
                                        ss->mouse_inputs.pos.y <= rightA.y + rightA.h);
                        if (inLeft) {
                            if (b.id == 400)
                                ss->menu.video_res_index = (ss->menu.video_res_index + 3) % 4;
                            else if (b.id == 401)
                                ss->menu.window_mode_index = (ss->menu.window_mode_index + 2) % 3;
                            else if (b.id == 403)
                                ss->menu.frame_limit_index = (ss->menu.frame_limit_index + 5) % 6;
                            else if (b.id == 405 && b.enabled)
                                ss->menu.window_size_index = (ss->menu.window_size_index + 3) % 4;
                            if (aa)
                                play_sound("base:ui_left");
                        } else if (inRight) {
                            if (b.id == 400)
                                ss->menu.video_res_index = (ss->menu.video_res_index + 1) % 4;
                            else if (b.id == 401)
                                ss->menu.window_mode_index = (ss->menu.window_mode_index + 1) % 3;
                            else if (b.id == 403)
                                ss->menu.frame_limit_index = (ss->menu.frame_limit_index + 1) % 6;
                            else if (b.id == 405 && b.enabled)
                                ss->menu.window_size_index = (ss->menu.window_size_index + 1) % 4;
                            if (aa)
                                play_sound("base:ui_right");
                        } else {
                        ...
                        if (b.id == 405 && b.enabled && gg && gg->window) {
                            ...
                            gg->window_dims =
                                glm::uvec2{(unsigned)kResW[idx], (unsigned)kResH[idx]};
                        }
                    }
            ...
            if (b.kind == ButtonKind::Slider) {
                if (ss->mouse_inputs.left) {
                    if (!ss->menu.ignore_mouse_until_release) {
                        ...
                        t = std::clamp(t, 0.0f, 1.0f);
                        if (b.id == 300)
                            ss->menu.vol_master = t;
                        else if (b.id == 301)
                            ss->menu.vol_music = t;
                        else if (b.id == 302)
                            ss->menu.vol_sfx = t;
                        else if (b.id == 500)
                            ss->menu.screen_shake = t;
                        else if (b.id == 501)
                            ss->menu.mouse_sens = t;
                        else if (b.id == 502)
                            ss->menu.controller_sens = t;
                        else if (b.id == 404)
                            ss->menu.ui_scale = t;
                        else if (b.id == 507)
                            ss->menu.vibration_magnitude = t;
                    }
                }
            }
        }
    ...
    if (fid >= 0) {
        if (nav_up) {
            ...
            if (b && b->kind == ButtonKind::Slider) {
                float* pv = nullptr;
                if (fid == 300)
                    pv = &ss->menu.vol_master;
                else if (fid == 301)
                    pv = &ss->menu.vol_music;
                else if (fid == 302)
                    pv = &ss->menu.vol_sfx;
                else if (fid == 500)
                    pv = &ss->menu.screen_shake;
                else if (fid == 501)
                    pv = &ss->menu.mouse_sens;
                else if (fid == 502)
                    pv = &ss->menu.controller_sens;
                else if (fid == 404)
                    pv = &ss->menu.ui_scale;
                else if (fid == 507)
                    pv = &ss->menu.vibration_magnitude;
                if (pv) {
                    if (fid == 501 || fid == 502) {
                        double step_val = 0.01;
                        double minv = 0.10, maxv = 10.0;
                        double range = maxv - minv;
                        double tstep = step_val / range;
                        *pv = std::clamp(*pv - (float)tstep, 0.0f, 1.0f);
                    } else {
                        *pv = std::clamp(*pv - 0.05f, 0.0f, 1.0f);
                    }
                    if (aa)
                        play_sound("base:ui_left");
                }
        ...
        if (ss->menu_inputs.confirm) {
            ...
            if (b) {
                ...
                static const char* nouns[] = {
                    "pickle",  "sausage",  "donut",      "potato",   "goblin", "monkey",
                    "hamster", "toaster",  "banana",     "slime",    "frog",   "llama",
                    "raccoon", "duck",     "waffle",     "muffin",   "taco",   "snail",
                    "jelly",   "robot",    "chicken",    "wizard",   "shrimp", "onion",
                    "cactus",  "pirate",   "pigeon",     "chair",    "noodle", "burger",
                    "clown",   "wizard",   "gnome",      "sock",     "crab",   "pancake",
                    "fish",    "sandwich", "gobstopper", "eggplant", "troll"};
                static const char* adjs[] = {
                    "wonky",  "wobbly", "smelly", "sneaky", "chonky", "crispy", "bouncy",
                    "soggy",  "tiny",   "spiky",  "grumpy", "silly",  "turbo",  "funky",
                    "flappy", "derpy",  "fizzy",  "greasy", "cosmic", "loopy",  "cranky"};
                std::mt19937 rng{std::random_device{}()};
                std::uniform_int_distribution<int> dn(0,
                                                      (int)(sizeof(nouns) / sizeof(nouns[0])) - 1);
                std::uniform_int_distribution<int> da(0, (int)(sizeof(adjs) / sizeof(adjs[0])) - 1);
                std::string base;
                for (int attempts = 0; attempts < 5; ++attempts) {
                    base = std::string(adjs[da(rng)]) + " " + nouns[dn(rng)];
                    bool clash = false;
                    for (auto const& n : exists)
                        if (n == base) {
                            clash = true;
                            break;
                        }
                ...
                auto present = [&](const std::string& s) {
                    for (auto const& n : exists)
                        if (n == s)
                            return true;
                    return false;
                ...
                if (save_input_bindings_to_ini(std::string("binds/") + name + ".ini",
                                               ss->input_binds)) {
                    ...
                    if (aa)
                        play_sound("base:ui_super_confirm");
                }
            } else if (ss->menu.page == BINDS && b->id == 799) {
                ss->menu.page = SETTINGS;
                ss->menu.focus_id = -1;
                ...
                if (!ss->menu.binds_current_preset.empty()) {
                    save_input_bindings_to_ini(std::string("binds/") +
                                                   ss->menu.binds_current_preset + ".ini",
                                               ss->input_binds);
                }
                ss->menu.binds_dirty = false;
                if (aa)
    ...
    if (ss->menu_inputs.back) {
        ...
        } else if (ss->menu.page == AUDIO || ss->menu.page == VIDEO || ss->menu.page == CONTROLS ||
                   ss->menu.page == BINDS || ss->menu.page == OTHER) {
            ss->menu.page = SETTINGS;
            ss->menu.focus_id = -1;
            ss->menu.capture_action_id = -1;
        }
    ...
    if (ss->menu.page == BINDS) {
        ...
        if (ss->menu.focus_id >= 700 && ss->menu.focus_id <= 704) {
            if (ss->menu_inputs.page_prev || left_edge_now) {
                ss->menu.binds_keys_page = std::max(0, ss->menu.binds_keys_page - 1);
                if (aa)
                    play_sound("base:ui_left");
            } else if (ss->menu_inputs.page_next || right_edge_now) {
                ss->menu.binds_keys_page = std::min(total_pages - 1, ss->menu.binds_keys_page + 1);
                if (aa)
                    play_sound("base:ui_right");
            }
        }
        ...
        SDL_Rect nextb{width - 120, hdr_y - 4, 90, 32};
        if (click) {
            int mx_i = ss->mouse_inputs.pos.x;
            int my_i = ss->mouse_inputs.pos.y;
            auto in_rect = [&](SDL_Rect rc) {
                return mx_i >= rc.x && mx_i <= rc.x + rc.w && my_i >= rc.y && my_i <= rc.y + rc.h;
            };
            if (in_rect(prevb)) {
                ss->menu.binds_keys_page = std::max(0, ss->menu.binds_keys_page - 1);
                if (aa)
        ...
        int per_page = 5;
        int total_pages =
            std::max(1, (int)((presets.size() + (size_t)per_page - 1) / (size_t)per_page));
        if (ss->menu.focus_id >= 720 && ss->menu.focus_id <= 724) {
            if (ss->menu_inputs.page_prev || left_edge_now) {
                ss->menu.binds_list_page = std::max(0, ss->menu.binds_list_page - 1);
                if (aa)
                    play_sound("base:ui_left");
            } else if (ss->menu_inputs.page_next || right_edge_now) {
                ss->menu.binds_list_page = std::min(total_pages - 1, ss->menu.binds_list_page + 1);
                if (aa)
                    play_sound("base:ui_right");
            }
        }
            ...
            if (fid2 >= 720 && fid2 <= 724) {
                ...
                if (idx >= 0 && idx < (int)presets.size()) {
                    ...
                    if (load_input_bindings_from_ini(std::string("binds/") + name + ".ini")) {
                        ...
                        ss->menu.focus_id = -1;
                        ss->alerts.push_back(
                            {std::string("Loaded preset: ") + name, 0.0f, 1.2f, false});
                        if (aa)
                            play_sound("base:ui_confirm");
                    }
                }
