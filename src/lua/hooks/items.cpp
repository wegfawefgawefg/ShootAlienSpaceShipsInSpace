...
bool LuaManager::call_item_on_use(int item_type, Entity& player, std::string* out_msg) {
    ...
    if (it == hooks_->items.end() || !it->second.on_use.valid())
        return false;
...
void LuaManager::call_item_on_tick(int item_type, Entity& player, float dt) {
    ...
    if (!def)
        return;
    auto it = hooks_->items.find(item_type);
    if (it == hooks_->items.end() || !it->second.on_tick.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
...
void LuaManager::call_item_on_shoot(int item_type, Entity& player) {
    ...
    if (!def)
        return;
    auto it = hooks_->items.find(item_type);
    if (it == hooks_->items.end() || !it->second.on_shoot.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
...
void LuaManager::call_item_on_damage(int item_type, Entity& player, int attacker_ap) {
    ...
    if (!def)
        return;
    auto it = hooks_->items.find(item_type);
    if (it == hooks_->items.end() || !it->second.on_damage.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
...
void LuaManager::call_item_on_pickup(int item_type, Entity& player) {
    ...
    auto it = hooks_->items.find(item_type);
    if (!def || it == hooks_->items.end() || !it->second.on_pickup.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it->second.on_pickup();
    if (!r.valid()) {
        sol::error e = r;
...
void LuaManager::call_item_on_drop(int item_type, Entity& player) {
    ...
    if (!def || it2 == hooks_->items.end() || !it2->second.on_drop.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it2->second.on_drop();
    if (!r.valid()) {
        sol::error e = r;
...
void LuaManager::call_item_on_active_reload(int item_type, Entity& player) {
    ...
    if (!def || it3 == hooks_->items.end() || !it3->second.on_active_reload.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it3->second.on_active_reload();
    if (!r.valid()) {
        sol::error e = r;
...
void LuaManager::call_item_on_failed_active_reload(int item_type, Entity& player) {
    ...
    if (!def || it4 == hooks_->items.end() || !it4->second.on_failed_active_reload.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it4->second.on_failed_active_reload();
    if (!r.valid()) {
        sol::error e = r;
...
void LuaManager::call_item_on_tried_after_failed_ar(int item_type, Entity& player) {
    ...
    if (!def || it5 == hooks_->items.end() || !it5->second.on_tried_after_failed_ar.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it5->second.on_tried_after_failed_ar();
    if (!r.valid()) {
        sol::error e = r;
        std::fprintf(stderr, "[lua] item on_tried_after_failed_ar error: %s\n", e.what());
    }
}
...
void LuaManager::call_item_on_eject(int item_type, Entity& player) {
    ...
    if (!def || it6 == hooks_->items.end() || !it6->second.on_eject.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it6->second.on_eject();
    if (!r.valid()) {
        sol::error e = r;
...
void LuaManager::call_item_on_reload_start(int item_type, Entity& player) {
    ...
    if (!def || it7 == hooks_->items.end() || !it7->second.on_reload_start.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it7->second.on_reload_start();
    if (!r.valid()) {
        sol::error e = r;
...
void LuaManager::call_item_on_reload_finish(int item_type, Entity& player) {
    ...
    if (!def || it8 == hooks_->items.end() || !it8->second.on_reload_finish.valid())
        return;
    LuaCtxGuard _ctx(ss, &player);
    auto r = it8->second.on_reload_finish();
    if (!r.valid()) {
        sol::error e = r;
