...
void generate_room() {
    ...
    // Spawn a few Lua-defined pickups/items near start for testing
    {
        ...
        if (luam && !luam->powerups().empty()) {
            ...
            auto* p =
                ss->pickups.spawn(static_cast<uint32_t>(pu.type), pu.name, place({1.0f, 0.0f}));
            if (p) {
        ...
        if (luam && luam->powerups().size() > 1) {
            ...
            auto* p =
                ss->pickups.spawn(static_cast<uint32_t>(pu.type), pu.name, place({0.0f, 1.0f}));
            if (p) {
                if (!pu.sprite.empty() && pu.sprite.find(':') != std::string::npos)
                    p->sprite_id = try_get_sprite_id(pu.sprite);
                else
        ...
        if (luam && ss->player_vid) {
                        ...
                        if (auto gv = ss->guns.spawn_from_def(g)) {
                            if (ss->player_vid)
                                if (auto* inv = ss->inv_for(*ss->player_vid))
                                    inv->insert_existing(INV_GUN, *gv);
                            return *gv;
                        }
                    }
                }
            ...
            if (v1.id || v2.id || v3.id) {
                ...
                if (v1.id)
                    p->equipped_gun_vid = v1;
                else if (v2.id)
                    p->equipped_gun_vid = v2;
                else if (v3.id)
                    p->equipped_gun_vid = v3;
            }
        }
