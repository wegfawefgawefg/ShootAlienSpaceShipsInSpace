    ...
    if (now_pick && !prev_pick && ss->pickup_lockout == 0.0f) {
        ...
        if (best_kind == PickKind::Gun) {
            ...
            if (auto* inv = (ss->player_vid ? ss->inv_for(*ss->player_vid) : nullptr))
                ok = inv->insert_existing(INV_GUN, ggun.gun_vid);
            std::string nm = "gun";
            if (luam)
                ...
                if (const GunInstance* ggi = ss->guns.get(ggun.gun_vid)) {
                    ...
                    if (luam && ss->player_vid)
                        if (auto* plent = ss->entities.get_mut(*ss->player_vid))
                            luam->call_gun_on_pickup(ggi->def_type, *plent);
                    if (gd)
                        play_sound(gd->sound_pickup.empty() ? "base:drop" : gd->sound_pickup);
                    else
                        play_sound("base:drop");
                }
            } else {
            ...
            if (pick) {
                if (auto* inv = (ss->player_vid ? ss->inv_for(*ss->player_vid) : nullptr))
                    for (auto& e : inv->entries) {
                        ...
                        tgt->count += xfer;
                        if (auto* pmut = ss->items.get(gi.item_vid))
                            pmut->count -= xfer;
            ...
            if (!fully_merged) {
                bool ok = false;
                if (auto* inv = (ss->player_vid ? ss->inv_for(*ss->player_vid) : nullptr))
                    ok = inv->insert_existing(INV_ITEM, gi.item_vid);
                if (ok) {
                    gi.active = false;
                    did_pick = true;
                    ss->alerts.push_back({std::string("Picked up ") + nm, 0.0f, 2.0f, false});
                    if (luam && pick && ss->player_vid)
                        if (auto* plent = ss->entities.get_mut(*ss->player_vid))
                            luam->call_item_on_pickup(pick->def_type, *plent);
                    if (luam && pick) {
                        ...
                        if (idf)
                            play_sound(idf->sound_pickup.empty() ? "base:drop" : idf->sound_pickup);
                        else
                            play_sound("base:drop");
                    }
                    if (ss->player_vid)
        ...
        if (did_pick)
            ss->pickup_lockout = PICKUP_DEBOUNCE_SECONDS;
    }
    prev_pick = now_pick;
}
                            ...
                            if (const ItemInstance* inst = ss->items.get(ent->vid)) {
                                ...
                                if (ss->player_vid)
                                    if (auto* pm = ss->metrics_for(*ss->player_vid))
                                        pm->items_dropped += 1;
                                ss->alerts.push_back(
                                    {std::string("Dropped item: ") + nm, 0.0f, 2.0f, false});
                            }
                        }
                    }
                }
                        ...
                        if (Entity* p = ss->entities.get_mut(*ss->player_vid)) {
                            ...
                            if (luam) {
                                if (const GunInstance* gi = ss->guns.get(ent->vid)) {
                                    for (auto const& g : luam->guns())
                                        if (g.type == gi->def_type) {
                                            ss->alerts.push_back({std::string("Equipped ") + g.name,
                                                                  0.0f, 1.2f, false});
                                            break;
                                        }
                                }
