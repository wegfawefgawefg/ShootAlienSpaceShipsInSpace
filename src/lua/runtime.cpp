...
bool LuaManager::load_mods() {
        ...
        if (fs::exists(sdir, ec) && fs::is_directory(sdir, ec)) {
            ...
            if (vobj.is<sol::table>()) {
                ...
                if (major != lua_api::kApiMajor) {
                    std::fprintf(stderr,
                                 "[lua] [%s] api_version mismatch: mod=%d.%d engine=%d.%d (major "
                                 "must match)\n",
                                 mod.path().filename().string().c_str(), major, minor,
                                 lua_api::kApiMajor, lua_api::kApiMinor);
                } else if (minor > lua_api::kApiMinor) {
                    std::fprintf(stderr,
                                 "[lua] [%s] api_version minor too new: mod=%d.%d engine=%d.%d "
    ...
    std::printf("[lua] loaded: %zu powerups, %zu items, %zu guns, %zu ammo, %zu projectiles, %zu "
                "entity types\n",
