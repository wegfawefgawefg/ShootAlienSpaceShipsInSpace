...
void update_exit_countdown() {
    if (!ss || ss->mode != ids::MODE_PLAYING)
        return;
...
void start_score_review_if_ready() {
    ...
    if (ss->exit_countdown >= 0.0f) {
        ...
        if (ss->exit_countdown <= 0.0f) {
            ...
            std::uint64_t total_powerups_picked = 0, total_items_picked = 0, total_guns_picked = 0,
                          total_items_dropped = 0, total_guns_dropped = 0, total_damage_dealt = 0;
            ...
            std::int64_t missed_powerups =
                (std::int64_t)ss->metrics.powerups_spawned - (std::int64_t)total_powerups_picked;
            std::int64_t missed_items =
                (std::int64_t)ss->metrics.items_spawned - (std::int64_t)total_items_picked;
            std::int64_t missed_guns =
                (std::int64_t)ss->metrics.guns_spawned - (std::int64_t)total_guns_picked;
            if (missed_powerups < 0)
                missed_powerups = 0;
            if (missed_items < 0)
                missed_items = 0;
            if (missed_guns < 0)
            ...
            double acc_total = total_shots_fired
                                   ? (100.0 * (double)total_shots_hit / (double)total_shots_fired)
                                   : 0.0;
            ...
            for (auto const& e : ss->entities.data()) {
                ...
                double acc = pm->shots_fired
                                 ? (100.0 * (double)pm->shots_hit / (double)pm->shots_fired)
                                 : 0.0;
...
void process_score_review_advance() {
    if (!ss || ss->mode != ids::MODE_SCORE_REVIEW)
        return;
...
void process_next_stage_enter() {
    if (!ss || ss->mode != ids::MODE_NEXT_STAGE)
        return;
    if (ss->score_ready_timer > 0.0f)
        return;
    if (ss->menu_inputs.confirm || ss->playing_inputs.use_center || ss->mouse_inputs.left) {
