...
void lua_register_api_world(sol::state& s, LuaManager& m) {
    ...
    api.set_function("spawn_entity_safe", [](int type, float x, float y) {
        ...
        if (!ed) {
            if (g_state_ctx)
                g_state_ctx->alerts.push_back(
                    {std::string("Unknown entity type ") + std::to_string(type), 0.0f, 1.5f,
                     false});
            return;
        }
        auto tile_blocks = [&](int tx, int ty) {
        ...
        if (!vid) {
            g_state_ctx->alerts.push_back({"Entity spawn failed", 0.0f, 1.5f, false});
            return;
        }
        ...
        e->stats.move_spread_inc_rate_deg_per_sec_at_base =
            ed->move_spread_inc_rate_deg_per_sec_at_base;
