        ...
        if (e.type_ == ids::ET_NPC || e.type_ == ids::ET_PLAYER) {
            ...
            if (luam && h.ammo_type != 0) {
                if (auto const* ad = luam->find_ammo(h.ammo_type)) {
                    if (ad->falloff_end > ad->falloff_start && ad->falloff_end > 0.0f) {
                        float m = 1.0f;
                        if (h.travel_dist <= ad->falloff_start)
                            m = 1.0f;
                        else if (h.travel_dist >= ad->falloff_end)
                            m = ad->falloff_min_mult;
                        else {
                            float t = (h.travel_dist - ad->falloff_start) /
                                      (ad->falloff_end - ad->falloff_start);
            ...
            if (e.type_ == ids::ET_PLAYER) {
                if (e.stats.shield_max > 0.0f && e.shield > 0.0f) {
                    ...
                    e.shield -= took;
                    if (auto* pm = ss->metrics_for(e.vid))
                        pm->damage_taken_shield += (std::uint64_t)std::lround(took);
                    if (h.owner)
                        if (auto* om = ss->metrics_for(*h.owner))
                ...
                if (dmg > 0.0f && e.stats.plates > 0) {
                    ...
                    if (e.stats.plates == 0 && luam && e.def_type)
                        luam->call_entity_on_plates_lost(e.def_type, e);
                    dmg = 0.0f;
                }
                if (luam && e.def_type)
                ...
                if (dmg > 0.0f) {
                    ...
                    if (luam && e.def_type && e.max_hp > 0) {
                        ...
                        if (prev >= 0.5f && now < 0.5f)
                            luam->call_entity_on_hp_under_50(e.def_type, e);
                        if (prev >= 0.25f && now < 0.25f)
                            luam->call_entity_on_hp_under_25(e.def_type, e);
                        if (now <= 0.0f) {
                        }
                    }
                }
                ...
                if (dmg > 0.0f) {
                    ...
                    e.health = (e.health > (uint32_t)delt) ? (e.health - (uint32_t)delt) : 0u;
                    if (h.owner)
                        if (auto* pm = ss->metrics_for(*h.owner))
            ...
            if (U(rng) < 0.5f && luam) {
                ...
                if (!dt.powerups.empty() || !dt.items.empty() || !dt.guns.empty()) {
                    ...
                    if (c < 0.5f && !dt.powerups.empty()) {
                        ...
                        if (t >= 0) {
                            auto it =
                                std::find_if(luam->powerups().begin(), luam->powerups().end(),
                                             [&](const PowerupDef& p) { return p.type == t; });
                            if (it != luam->powerups().end()) {
                                auto* p =
                                    ss->pickups.spawn((uint32_t)it->type, it->name, place_pos);
                                if (p) {
                        ...
                        if (t >= 0) {
                            ...
                            if (ig != luam->guns().end()) {
                                if (auto gv = ss->guns.spawn_from_def(*ig)) {
                                    ...
                                    ss->ground_guns.spawn(*gv, place_pos, sid);
